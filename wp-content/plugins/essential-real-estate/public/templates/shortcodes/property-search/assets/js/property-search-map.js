...
(function ($) {
    ...
    ERE_Property_Map_Search = {
        init: function () {
            ...
            $('.other-features-wrap .btn-other-features', css_class_wrap).on('click', function (event) {
                ...
                if ($(map_text).length > 0) {
                    ...
                    ERE_Property_Map_Search.search_on_change(current_form, map_id);
                }
            });
            $(".ere-sliderbar-filter.ere-sliderbar-price", css_class_wrap).on('register.again', function () {
                $(".ere-sliderbar-filter.ere-sliderbar-price", css_class_wrap).each(function () {
            ...
            $('.ere-search-properties').each(function () {
                ...
                if ($(map_id_text).length > 0) {
                    $('select[name="type"],select[name="status"],input[name="status"], select[name="bedrooms"],select[name="bathrooms"] , ' +
                    ...
                    $('input[name="other_features"]', this_form).on('change', function () {
                        var current_form = this_form.find('.search-properties-form');
                        ERE_Property_Map_Search.search_on_change(current_form, map_ID);
                    });
                    ERE_Property_Map_Search.search_properties('', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', map_ID, 'map_only');
                }
            })
        },
        get_states_by_country: function () {
            if ($('.ere-property-country-ajax', css_class_wrap).length) {
        ...
        get_cities_by_state: function () {
            if ($('.ere-property-state-ajax', css_class_wrap).length) {
                var selected_state = $(".ere-property-state-ajax", css_class_wrap).val();
                if (selected_state != '') {
                    $.ajax({
                        type: "POST",
                        url: ajax_url,
        ...
        get_neighborhoods_by_city: function () {
            if ($(".ere-property-city-ajax", css_class_wrap).length) {
        ...
        search_on_change: function (current_form, map_ID) {
            var country, city, state, neighborhood, title, area, status, type, bedrooms, bathrooms, min_price, max_price,
                min_area, max_area, address, garage, features, label, min_land_area, max_land_area, property_identity, features_enable, map_result;
            title = current_form.find('input[name="title"]').val();
            address = current_form.find('input[name="address"]').val();
            ...
            if ($('.ere-sliderbar-price', current_form).length) {
            ...
            if ($('.ere-sliderbar-area', current_form).length) {
                ...
                min_area = current_form.find('select[name="min-area"]').val();
                max_area = current_form.find('select[name="max-area"]').val();
            }

            if ($('.ere-sliderbar-land-area', current_form).length) {
                ...
                min_land_area = current_form.find('select[name="min-land-area"]').val();
                max_land_area = current_form.find('select[name="max-land-area"]').val();
            }
            ...
            features_enable = current_form.find('input[name="features-search"]').val();
            if (features_enable == '1') {
                features = '';
                current_form.find('.other-features-list input[type=checkbox]:checked').each(function () {
                    features += $(this).val() + ';';
                });
                if (features != '') {
            ...
            ERE_Property_Map_Search.search_properties(title, address, country, state, city, neighborhood, type, status, bedrooms, bathrooms, min_price, max_price,
                min_area, max_area, label, garage, min_land_area, max_land_area, property_identity, features, map_result, search_type);
        },
        search_properties: function (title, address, country, state, city, neighborhood, type, status, bedrooms, bathrooms, min_price, max_price, min_area, max_area, label, garage, min_land_area, max_land_area, property_identity, features, map_result, search_type) {
